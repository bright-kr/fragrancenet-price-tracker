"""
FragranceNet Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from FragranceNet.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/fragrancenet
"""
...
# Search for "FragranceNet" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
...
if not DATASET_ID:
    raise EnvironmentError(
        ...
        "2. Search for 'FragranceNet' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of FragranceNet product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover FragranceNet products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a FragranceNet category page.

    Example:
        results = discover_by_category("https://fragrancenet.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.fragrancenet.com/product/sample-item-123456",
        # Add more FragranceNet product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "fragrancenet_prices.json"
    ...
    # cat_results = discover_by_category("https://fragrancenet.com/category/sample", limit=50)
